function retCell = setTarget(passCell,freqx,matchMode,example)
...
% default values for the freq-dependant weighting functions
errWeights_ampl = ones(size(freqx));
errWeights_phase = ones(size(freqx));
target_ampl = ones(size(freqx));
target_phase = ones(size(freqx));
...
retCell = {};
retCell{1} = target;
%target = Htarget2; % for 2nd op-amp filter, which is 2nd order
% the combination should yield the original 5th-order filter
retCell{2} = errWeights;
